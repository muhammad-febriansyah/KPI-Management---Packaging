            ...
            return DataTables::eloquent($query)
                ...
                ->addColumn('action', function (User $user) use ($client): string {
                    ...
                    if ($user->is_super_admin) {
                        $buttons[] = '<button type="button" data-super-admin-edit data-url="'.route('settings.access.super-admins.update', $user).'" data-super-admin="'.e(json_encode($this->superAdminPayload($user))).'" class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-100"><svg aria-hidden="true" class="size-4 fill-none stroke-current"><use href="/images/heroicons.svg#pencil"></use></svg>Edit</button>';
                    } elseif ($user->role_code === 'employee' && $user->employee) {
                        $buttons[] = '<button type="button" data-employee-edit data-url="'.route('employees.update', $user->employee).'" data-employee="'.e($user->employee->toJson()).'" class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-100"><svg aria-hidden="true" class="size-4 fill-none stroke-current"><use href="/images/heroicons.svg#pencil"></use></svg>Edit</button>';
                    ...
                    if ($user->getKey() !== request()->user()?->getKey()) {
                        ...
                        $buttons[] = '<form method="POST" action="'.route('settings.access.users.destroy', $user).'" data-ajax-delete class="inline"><button type="submit" class="inline-flex items-center gap-1.5 rounded-lg bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 hover:bg-red-100"><svg aria-hidden="true" class="size-4 fill-none stroke-current"><use href="/images/heroicons.svg#trash"></use></svg>Hapus</button></form>';
                    }
